...
imagine(X,Cale,Descriere):- atom_concat('C:\\Users\\andreea\\Desktop\\pisici\\pisoi_',X,Calee),
							atom_concat(Calee,'.png',Cale),
							atom_concat('Descriere: O pisica ',X,Descriere).

proceseaza_termen_citit(Stream, img(X),C):-
				imagine(X,Cale,Descriere),
				format(Stream,'d(~s#~s)\n',[Cale,Descriere]),
				flush_output(Stream),
				C1 is C+1,
				proceseaza_text_primit(Stream,C1).
